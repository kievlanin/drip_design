        ...
        if name == "capture_tiles":
            ...
            if app is not None and len(getattr(app, "trunk_map_nodes", []) or []) > 0:
                hint.set(
                    ...
                    f"(лише по вузлах); пікет — окремою командою «Пікет». "
                    f"ПКМ — з’єднати з кінцем. Прив’язка ~{int(TRUNK_NODE_SNAP_M)} м. "
...
def main() -> None:
    ...
    _attach_dark_tooltip(
        _main_btn_trunk,
        "Труба: ЛКМ+ПКМ на вузлі — кінець ребра; ЛКМ — початок і трасувальні точки (лише по вузлах); "
        "пікет — окремою командою «Пікет»; "
